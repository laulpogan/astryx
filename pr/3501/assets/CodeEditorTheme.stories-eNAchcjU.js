import{a3 as e,aj as T,H as L}from"./iframe-CJeV72rj.js";import{a as b,c as C,b as N,d as v,g as D,e as P,m as U,n as G,o as H,f as w,s as M,t as A,h as _,S as k}from"./presets-DlGoEGXX.js";import{C as S}from"./CodeEditor-hbkVw4-e.js";import"./preload-helper-Ct5FWWRu.js";import"./highlightRanges-Bl51GkoT.js";const z=["import {useState, useEffect} from 'react';","","interface User {","  id: string;","  name: string;","  roles: string[];","}","",'const API_URL = "https://api.example.com";',"const MAX_RETRIES = 3;","","// Fetch user data with retry logic","async function fetchUser(id: string): Promise<User> {","  const response = await fetch(`${API_URL}/users/${id}`);","  if (!response.ok) {","    throw new Error(`HTTP ${response.status}`);","  }","  return response.json();","}","","export function UserCard({id}: {id: string}) {","  const [user, setUser] = useState<User | null>(null);","","  useEffect(() => {","    fetchUser(id).then(setUser);","  }, [id]);","","  if (!user) return <div>Loading...</div>;","","  return (",'    <div className="card">',"      <h2>{user.name}</h2>","      <span>{user.roles.length} roles</span>","    </div>","  );","}"].join(`
`);function r({theme:t,_title:x,initialCode:y=z}){const[E,j]=T.useState(y);return e.jsx(k,{theme:t,children:e.jsx(S,{value:E,onChange:j,language:"typescript",hasLineNumbers:!0})})}const X={title:"Lab/CodeEditorTheme",parameters:{docs:{description:{component:"Syntax theme showcase for CodeEditor. All themes from SyntaxTheme work identically on both CodeBlock and CodeEditor."}}}},s={render:()=>e.jsx(r,{theme:H})},a={render:()=>e.jsx(r,{theme:v})},o={render:()=>e.jsx(r,{theme:U})},n={render:()=>e.jsx(r,{theme:G})},c={render:()=>e.jsx(r,{theme:A})},d={render:()=>e.jsx(r,{theme:N})},i={render:()=>e.jsx(r,{theme:P})},m={render:()=>e.jsx(r,{theme:D})},p={render:()=>e.jsx(r,{theme:M})},u={render:()=>e.jsx(r,{theme:w})},h={render:()=>e.jsx(r,{theme:C})},l={render:()=>e.jsx(r,{theme:_})},O=["const greet = (name: string) => {","  // Say hello","  return `Hello, ${name}!`;","};","",'const result = greet("World");',"console.log(result); // Hello, World!"].join(`
